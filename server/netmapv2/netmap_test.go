...
func TestNetworkMap_UpdateLocalStatus(t *testing.T) {
	localNode := &Node{
		ID:     "local",
		Status: NodeStatusActive,
	}
	...
	var notifiedStatus NodeStatus
	m.OnLocalStatusUpdate(func(status NodeStatus) {
		notifiedStatus = status
	})
	m.UpdateLocalStatus(NodeStatusLeaving)

	assert.Equal(t, NodeStatusLeaving, notifiedStatus)

	n, _ := m.Node("local")
	assert.Equal(t, NodeStatusLeaving, n.Status)
}
...
func TestNetworkMap_UpdateRemoteStatus(t *testing.T) {
	t.Run("update status", func(t *testing.T) {
		...
		m.AddNode(newNode)
		m.UpdateRemoteStatus("remote", NodeStatusActive)

		n, _ := m.Node("remote")
		assert.Equal(t, NodeStatusActive, n.Status)
	})
	...
	t.Run("update local status", func(t *testing.T) {
		...
		m.UpdateRemoteStatus("local", NodeStatusLeaving)
		assert.Equal(t, localNode, m.LocalNode())
	})
}
...
func TestNetworkMap_UpdateRemoteEndpoint(t *testing.T) {
	t.Run("update endpoint", func(t *testing.T) {
		...
		m.AddNode(newNode)
		m.UpdateRemoteEndpoint("remote", "my-endpoint", 7)
	...
	t.Run("update local status", func(t *testing.T) {
		...
		// Attempting to update the local node should have no affect.
		m.UpdateRemoteEndpoint("local", "my-endpoint", 7)
		assert.Equal(t, localNode, m.LocalNode())
	})
}
...
func TestNetworkMap_RemoveRemoteEndpoint(t *testing.T) {
	t.Run("update endpoint", func(t *testing.T) {
		...
		m.AddNode(newNode)
		m.UpdateRemoteEndpoint("remote", "my-endpoint", 7)
		m.RemoveRemoteEndpoint("remote", "my-endpoint")
	...
	t.Run("update local status", func(t *testing.T) {
		...
		// Attempting to update the local node should have no affect.
		m.RemoveRemoteEndpoint("local", "my-endpoint")
		assert.Equal(t, localNode, m.LocalNode())
	})
}
